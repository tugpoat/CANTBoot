...
class NetComm:
	...
	def __init__(self):
		self.__logger = logging.getLogger("DIMMComm " + str(id(self)))

	def connect(self, ip, port) -> bool:

		if self.is_connected and self.s != None:
			self.__logger.debug("Called connect() with existant socket. Disconnecting.")
			self.disconnect()

		self.__logger.info("Connecting to " + ip + ":" + str(port)) 
		try:
			...
			self.__logger.error("Connection failed! " + repr(ex))
